...
function mundula_bump_customer_orders_cache_version() {
    $version = (int) get_option( 'mundula_coc_version', 1 );
    update_option( 'mundula_coc_version', $version + 1 );
}
add_action( 'save_post_mundula_zgloszenie', 'mundula_bump_customer_orders_cache_version' );
add_action( 'deleted_post',                 'mundula_bump_customer_orders_cache_version' );
...
add_action( 'admin_init', function() {
    if ( ! get_option( 'mundula_migration_wycena_status_v1' ) ) {
        global $wpdb;
        $wpdb->query("
            UPDATE {$wpdb->postmeta} pm
            INNER JOIN {$wpdb->posts} p ON p.ID = pm.post_id
            SET pm.meta_value = 'wycena'
            WHERE p.post_type = 'mundula_zgloszenie'
              AND pm.meta_key = '_status'
              AND pm.meta_value = 'rezerwacja'
              AND p.post_title LIKE 'Wycena%'
        ");
        update_option( 'mundula_migration_wycena_status_v1', 1 );
    }
});
