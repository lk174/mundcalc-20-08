...
function mundula_panel_cennik() {
    ...
    // Pogrupowanie płaskich stawek według zakładki
    $groups = [];
    foreach ( $stawki as $id => $item ) {
        $g = $item['g'] ?? 'Inne';
        $g_lower = mb_strtolower($g);
    ...
    // Tabele płaskie dla aktywnej zakładki
    ...
    <?php if ( empty($raw_pc_default) ): ?>
        <div class="notice notice-error"><p><strong>❌ Błąd:</strong> Nie znaleziono lub nie można odczytać pliku domyślnego <code>wp-content/kalkulator/mundula-price-change-default.json</code>.</p></div>
    <?php elseif ( ! isset($raw_pc_default['PRICE_CHANGE_PLASZCZ']) ): ?>
        <div class="notice notice-warning"><p><strong>⚠️ Ostrzeżenie:</strong> Plik <code>wp-content/kalkulator/mundula-price-change-default.json</code> na Twoim serwerze jest nieaktualny (brak nowych matryc). Upewnij się, że wgrałeś najnowszy plik z repozytorium na serwer FTP.</p></div>
    <?php endif; ?>

    <h2 class="nav-tab-wrapper" style="margin-bottom: 20px;">
        <?php foreach ($tabs as $tid => $tname): ?>
            <a href="?page=mundula-stawki&tab=<?= $tid ?>" class="nav-tab <?= $active_tab === $tid ? 'nav-tab-active' : '' ?>"><?= esc_html($tname) ?></a>
        <?php endforeach; ?>
    </h2>
    ...
    <?php if ($saved): ?>
    <div class="notice notice-success is-dismissible"><p>✅ Stawki zapisane.</p></div>
    <?php elseif ($reset): ?>
    <div class="notice notice-success is-dismissible"><p>✅ Stawki zresetowane do wartości domyślnych dla tej formacji.</p></div>
    <?php elseif (isset($_GET['defaults_saved'])): ?>
    <div class="notice notice-success is-dismissible"><p>✅ Bieżące stawki zostały zapisane jako nowe wartości domyślne (pliki JSON zaktualizowane).</p></div>
    <?php elseif (isset($_GET['defaults_saved_error'])): ?>
    <div class="notice notice-error is-dismissible"><p>❌ Błąd zapisu: Nie można zapisać plików JSON na serwerze (brak uprawnień zapisu w folderze wp-content/kalkulator/).</p></div>
    <?php endif; ?>

    <!-- Szybka nawigacja -->
    <div class="mundula-quick-links" style="margin: 15px 0 25px 0; display: flex; gap: 8px; flex-wrap: wrap;">
        <?php if ($active_tab === 'wp'): ?>
            <a href="#table-change-wp" class="button button-secondary button-small" style="font-size: 11px;">Marynarka</a>
            <a href="#table-change-plaszcz" class="button button-secondary button-small" style="font-size: 11px;">Płaszcz</a>
            <a href="#table-change-koszula" class="button button-secondary button-small" style="font-size: 11px;">Koszula</a>
            <a href="#table-change-czapka-gotowy" class="button button-secondary button-small" style="font-size: 11px;">Czapka (gotowa)</a>
            <a href="#table-change-czapka-bajorek" class="button button-secondary button-small" style="font-size: 11px;">Czapka (bajorek)</a>
            <a href="#table-change-beret-gal-gotowy" class="button button-secondary button-small" style="font-size: 11px;">Beret gal. (gotowy)</a>
            <a href="#table-change-beret-gal-bajorek" class="button button-secondary button-small" style="font-size: 11px;">Beret gal. (bajorek)</a>
            <a href="#table-change-beret-pol" class="button button-secondary button-small" style="font-size: 11px;">Beret polowy</a>
            <a href="#table-change-furazerka" class="button button-secondary button-small" style="font-size: 11px;">Furażerka</a>
            <a href="#table-change-kapelusz-gotowy" class="button button-secondary button-small" style="font-size: 11px;">Kapelusz (gotowy)</a>
            <a href="#table-change-kapelusz-bajorek" class="button button-secondary button-small" style="font-size: 11px;">Kapelusz (bajorek)</a>
            <a href="#table-change-pcr" class="button button-secondary button-small" style="font-size: 11px;">Podchorążowie (rok)</a>
        <?php elseif ($active_tab === 'sw'): ?>
            <a href="#table-change-sw" class="button button-secondary button-small" style="font-size: 11px;">Matryca Marynarka/Płaszcz</a>
        <?php elseif ($active_tab === 'sg'): ?>
            <a href="#table-change-sg" class="button button-secondary button-small" style="font-size: 11px;">Matryca Marynarka/Płaszcz</a>
        <?php elseif ($active_tab === 'psp'): ?>
            <a href="#table-change-psp" class="button button-secondary button-small" style="font-size: 11px;">Matryca Marynarka/Płaszcz</a>
        <?php endif; ?>

        <?php foreach ($active_groups as $gname => $items):
            $anchor = 'table-' . sanitize_title($gname);
        ?>
            <a href="#<?= $anchor ?>" class="button button-secondary button-small" style="font-size: 11px;"><?= esc_html($gname) ?></a>
        <?php endforeach; ?>
    </div>
    ...
    <form id="mundula-cennik-form" method="post" action="<?php echo admin_url('admin-post.php'); ?>">
        ...
        <!-- MATRYCE ZMIAN STOPNI -->
        <?php if ($active_tab === 'wp'): ?>
            <div id="table-change-wp" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE',     'pc',    'ST',     'ST',     'Wojsko Polskie / Siły Powietrzne — Marynarka', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-plaszcz" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_PLASZCZ', 'pc_plaszcz', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Płaszcz', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-koszula" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_KOSZULA', 'pc_koszula', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Koszula / Naramienniki', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-czapka-gotowy" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_CZAPKA_GOTOWY', 'pc_czapka_gotowy', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Czapka (Otok gotowy)', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-czapka-bajorek" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_CZAPKA_BAJOREK', 'pc_czapka_bajorek', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Czapka (Otok bajorek)', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-beret-gal-gotowy" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_BERET_GAL_GOTOWY', 'pc_beret_gal_gotowy', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Beret galowy (Oznaka gotowa)', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-beret-gal-bajorek" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_BERET_GAL_BAJOREK', 'pc_beret_gal_bajorek', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Beret galowy (Bajorek)', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-beret-pol" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_BERET_POL', 'pc_beret_pol', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Beret polowy', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-furazerka" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_FURAZERKA', 'pc_furazerka', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Furażerka', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-kapelusz-gotowy" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_KAPELUSZ_GOTOWY', 'pc_kapelusz_gotowy', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Kapelusz Podhalański (Tkanina)', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-kapelusz-bajorek" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_KAPELUSZ_BAJOREK', 'pc_kapelusz_bajorek', 'ST', 'ST', 'Wojsko Polskie / Siły Powietrzne — Kapelusz Podhalański (Bajorek)', $pc_data, $pc_default); ?>
            </div>
            <div id="table-change-pcr" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_ROK', 'pcr',   'ROK',    'ROK_L',  'Podchorążowie — zmiana roku nauki', $pc_data, $pc_default); ?>
            </div>
        <?php elseif ($active_tab === 'sw'): ?>
            <div id="table-change-sw" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_SW',  'pcsw',  'SW_ST',  'SW_ST',  'Służba Więzienna — zmiana dystynkcji (marynarka/płaszcz)', $pc_data, $pc_default); ?>
            </div>
        <?php elseif ($active_tab === 'sg'): ?>
            <div id="table-change-sg" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_SG',  'pcsg',  'SG_ST',  'SG_ST',  'Straż Graniczna — zmiana dystynkcji (marynarka/płaszcz)', $pc_data, $pc_default); ?>
            </div>
        <?php elseif ($active_tab === 'psp'): ?>
            <div id="table-change-psp" style="scroll-margin-top: 40px;">
                <?php mundula_render_pc_table('PRICE_CHANGE_PSP', 'pcpsp', 'PSP_ST', 'PSP_ST', 'Państwowa Straż Pożarna — zmiana dystynkcji (marynarka/płaszcz)', $pc_data, $pc_default); ?>
            </div>
        <?php endif; ?>

        <!-- PŁASKIE TABELE STAWEK I CENNIKI STOPNI ELEMENTÓW -->
        <?php foreach ($active_groups as $gname => $items):
            $anchor = 'table-' . sanitize_title($gname);
        ...
        <div id="<?= $anchor ?>" style="background:#fff;border:1px solid <?= $border_color ?>;border-radius:4px;margin-bottom:20px;scroll-margin-top: 40px;">
            ...
            <table class="widefat" style="border:none;">
                ...
                <?php foreach ($items as $sid => $item):
                    $current  = (int)$item['p'];
                    $def_val  = (int)($default_raw_val  = $default_stawki[$sid]['p'] ?? $current);
