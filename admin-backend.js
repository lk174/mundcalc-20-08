...
jQuery(document).ready(function($){
        ...
        // Natychmiastowy upload zdjęć przez AJAX
        $('#mundula-photos-input').on('change', function(e) {
            ...
            var fd = new FormData();
            fd.append('action', 'mundula_upload_photo');
            fd.append('nonce', nonce);
            fd.append('post_id', post_id);
            
            for (var i = 0; i < files.length; i++) {
                fd.append('mundula_photos[]', files[i]);
            }

            var $photosGrid = $('#mundula-photos-grid');
            var $noPhotosMsg = $('#mundula-no-photos-message');
            
            if ($photosGrid.length === 0) {
                if ($noPhotosMsg.length) $noPhotosMsg.hide();
                $('#mundula-photos-card-inside').prepend('<div id="mundula-photos-grid" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap:12px; margin-bottom:16px;"></div>');
                $photosGrid = $('#mundula-photos-grid');
            }

            // Wstaw bloczki ładowania
            var loaders = [];
            for (var i = 0; i < files.length; i++) {
                var loaderId = 'loader_' + Math.random().toString(36).substr(2, 9);
                loaders.push(loaderId);
                var loaderHtml = '<div id="' + loaderId + '" style="position:relative; border:1px dashed #7c3aed; border-radius:4px; padding:4px; background:#f5f3ff; text-align:center; height:120px; display:flex; flex-direction:column; justify-content:center; align-items:center;">' +
                    '<div style="font-size:10px; color:#7c3aed; font-weight:600;">Wysyłam...</div>' +
                    '</div>';
                $photosGrid.append(loaderHtml);
            }
            ...
            $.ajax({
                ...
                processData: false,
                success: function(r) {
                    $.each(loaders, function(idx, id) {
                        $('#' + id).remove();
                    });

                    if (r.success && r.data.photos) {
                        $.each(r.data.photos, function(idx, p) {
                            var cardHtml = '<div style="position:relative; border:1px solid #ddd; border-radius:4px; padding:4px; background:#fff; text-align:center;">' +
                                '<a href="' + p.view_url + '" target="_blank" title="Powiększ zdjęcie">' +
                                '<img src="' + p.view_url + '" style="width:100%; height:80px; object-fit:cover; border-radius:2px;" alt="' + p.name + '">' +
                                '</a>' +
                                '<div style="font-size:10px; color:#888; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-top:4px;" title="' + p.name + '">' +
                                p.name +
                                '</div>' +
                                '<a href="' + p.delete_url + '" class="submitdelete deletion" onclick="return confirm(\'Czy na pewno chcesz usunąć to zdjęcie?\')" style="color:#dc2626; font-size:11px; text-decoration:none; display:inline-block; margin-top:4px;">' +
                                '🗑️ Usuń' +
                                '</a>' +
                                '</div>';
                            $photosGrid.append(cardHtml);
                        });
                        mundulaShowToast("Zdjęcia zostały dodane.");
                    } else {
                        alert('Błąd: ' + (r.data && r.data.message ? r.data.message : 'nieznany'));
                    }
                },
                error: function() {
                    ...
                    alert('Błąd połączenia podczas wysyłania zdjęć.');
                }
            });
