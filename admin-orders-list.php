...
function mundula_panel_list() {
    ...
    if ( $filter_status ) {
        ...
        $allowed_statuses = array_keys( $MUNDULA_STATUSY );
        if ( $hide_wyslane ) {
            $allowed_statuses = array_diff( $allowed_statuses, [ 'wyslane_odebrane' ] );
        }
        $meta_query[] = [
            'key'     => '_status',
            'value'   => array_values( $allowed_statuses ),
            'compare' => 'IN',
        ];
    }
    ...
    if ( $filter_search ) {
        ...
        // Zapytanie wyszukujące pasujące klucze metadanych
        $meta_sql = "
            SELECT DISTINCT post_id 
            FROM {$wpdb->postmeta} 
            WHERE meta_key IN ($placeholders) 
              AND LCASE(meta_value) LIKE %s
        ";
        $meta_params = array_merge( $meta_keys, [ $search_like ] );
        
        // Uwzględnienie wyszukiwania znormalizowanego numeru telefonu, jeśli wpisano cyfry
        if ( $s_digits !== '' ) {
            $phone_keys = [ '_telefon', '_zbiorcze_telefon', '_kontakt' ];
            $phone_placeholders = implode( ',', array_fill( 0, count( $phone_keys ), '%s' ) );
            
            $meta_sql .= "
                OR (
                    meta_key IN ($phone_placeholders)
                    AND REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(meta_value, ' ', ''), '-', ''), '(', ''), ')', ''), '+', ''), '.', '') LIKE %s
                )
            ";
            $meta_params = array_merge( $meta_params, $phone_keys, [ '%' . $s_digits . '%' ] );
        }
        
        $matching_meta_ids = $wpdb->get_col( $wpdb->prepare( $meta_sql, $meta_params ) );
        
        // Dopasowanie do tytułu wpisu
        $matching_title_ids = $wpdb->get_col( $wpdb->prepare( "
            SELECT ID 
            FROM {$wpdb->posts} 
            WHERE post_type = 'mundula_zgloszenie' 
              AND LCASE(post_title) LIKE %s
        ", $search_like ) );
        
        $matching_ids = array_unique( array_merge( $matching_meta_ids, $matching_title_ids ) );
        
        if ( ! empty( $matching_ids ) ) {
            $args['post__in'] = $matching_ids;
        } else {
            $args['post__in'] = [ 0 ]; // Wymusza brak wyników, jeśli nic nie pasuje
        }
    }
    ...
    <!-- STATYSTYKI -->
    <?php
    // Zlicz bezpośrednio z bazy - świeże dane bez cache i bez N+1 query
    $counts = [];
    ...
    $status_counts = $wpdb->get_results("
        ...
        FROM {$wpdb->posts} p
        LEFT JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id AND pm.meta_key = '_status'
        WHERE p.post_type = 'mundula_zgloszenie'
          AND p.post_status = 'publish'
        GROUP BY pm.meta_value
    ", ARRAY_A);
    foreach ( $status_counts as $row ) {
        $st = $row['status'] ?: MUNDULA_STATUS_REZERWACJA;
        if ( isset($counts[$st]) ) {
